  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <div className="flex items-center justify-between py-4 px-6 border-b border-ink-800 shrink-0">
          <button
          ...
          <div className="max-w-3xl mx-auto py-6 space-y-4">
                    ...
                    {!isUser && msg.citations && msg.citations.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-ink-700">
                        <details className="text-xs text-ink-300">
                          <summary className="cursor-pointer font-medium hover:text-ink-200">
                            ...
                            {msg.citations.map((citation, idx) => {
                              if (citation.type === "legal") {
                                ...
                                return (
                                  <li key={idx} className="flex flex-col gap-1">
                                    <div className="flex items-center gap-2">
                                      <span className="px-2 py-0.5 rounded bg-sage-500/20 text-sage-400 text-xs font-medium">
                                        Your Document
                                      </span>
                                      <span>• {citation.title}</span>
                                    </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="inline-flex items-center gap-2 rounded-2xl bg-ink-900 text-ink-300 border border-ink-800 px-4 py-3 text-sm">
        ...
        <div className="border-t border-ink-800 px-4 py-4 shrink-0">
          <div className="max-w-3xl mx-auto">
            <form onSubmit={handleSend} className="w-full">
              <div className="flex items-center gap-3 bg-ink-900 rounded-full px-4 py-3 border border-ink-800">
                <input
                  className="flex-1 bg-transparent text-sm md:text-base text-ink-100 placeholder:text-ink-500 outline-none"
                ...
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-full bg-leather-500 hover:bg-leather-600 disabled:opacity-60 disabled:hover:bg-leather-500 text-ink-50 text-sm font-semibold transition-colors"
                >
                  ➤
                </button>
              </div>
        ...
        <style jsx>{`
          .custom-scroll {
            ...
            scrollbar-color: rgba(138, 118, 90, 0.25) transparent;
          }
          ...
          .custom-scroll::-webkit-scrollbar-thumb {
            background: rgba(138, 118, 90, 0.25);
            border-radius: 9999px;
          }

          .custom-scroll:hover::-webkit-scrollbar-thumb {
            background: rgba(138, 118, 90, 0.4);
          }
        `}</style>
      </div>
    </div>
